...
Actually the only bug that can cause it to crash is adding *w<span>ww.* when using the */link* command. Maybe there a few more bugs that haven't came up by testing. <br />
However it should be always restarted automatically every 5 minutes after the midnight of the day you've set up it, so if it crashes just wait a little. <br />
